...
train_data, test_data = split_by_patient_id(
    data, TEST_SIZE, SEED)
...
train_data = [
    ...
    for pid, img, label in tqdm(
        train_data, desc="CLAHE train")
]
test_data = [
    (pid, apply_clahe(img), label)
    for pid, img, label in tqdm(
        test_data, desc="CLAHE test")
]
ram = psutil.virtual_memory().used / 1e9
...
def augment_for_deit(train_data):
    ...
    for pid, img, label in tqdm(
            train_data, desc="Augmenting"):
        augmented.append((f"{pid}_hf",
            cv2.flip(img, 1), label))
        augmented.append((f"{pid}_vf",
            cv2.flip(img, 0), label))
        augmented.append((f"{pid}_stain1",
            augment_stain(img), label))
        augmented.append((f"{pid}_stain2",
            augment_stain(img), label))
...
if ram > 13:
    raise MemoryError(
        f"RAM too high: {ram:.1f} GB — reduce augmentation")
print("✅ RAM safe")
...
train_dataset = MIASDataset(
    train_data, deit_train_transforms, IMAGE_SIZE)
test_dataset  = MIASDataset(
    test_data, deit_test_transforms, IMAGE_SIZE)
...
with mlflow.start_run(run_name="deit_small_distilled"):
    ...
    deit_history, deit_best_epoch, deit_train_time = \
        train_deit(
            model=deit_model,
...
ax1.plot(epochs_range, deit_history["train_loss"],
         "b-", linewidth=2, label="Train Loss")
ax1.plot(epochs_range, deit_history["val_loss"],
         "r-", linewidth=2, label="Val Loss")
ax1_twin = ax1.twinx()
ax1_twin.plot(epochs_range, deit_history["val_auc"],
              "g--", linewidth=2, label="Val AUC")
ax1_twin.set_ylabel("AUC", color="green")
ax1.axvline(x=deit_best_epoch, color="gold",
...
ax3 = axes[2]
mnames  = ["AUC","Accuracy","F1",
           "Precision","Recall","Specificity"]
mvalues = [auc_score, accuracy, f1,
           precision, recall, specificity]
